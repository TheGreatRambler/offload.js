...
var offload = function(options) {
	...
	this.serverFiles = {};
	var filesToLoad = fs.readdirSync(path.join(__dirname, "server") + path.sep); // hardcoded
	readMultipleFiles(filesToLoad).then(function(fileBuffers) {
		fileBuffers.forEach(function(buffer, index) {
			self.serverFiles[path.basename(filesToLoad[index])] = buffer;
		});
		self._startServer();
	}, function(err) {
